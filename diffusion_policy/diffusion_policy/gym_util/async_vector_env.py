...
class AsyncVectorEnv(VectorEnv):
    ...
    def reset_async(self, seed=None, options=None):
        # gym >=0.26's VectorEnv.reset() forwards seed/options to reset_async/reset_wait.
        # This class predates that API; seeding is handled by the runner through
        # env_init_fn_dills, and callers here use a bare reset(), so both are always None.
        self._assert_is_running()
        if self._state != AsyncState.DEFAULT:
            raise AlreadyPendingCallError(
    ...
    def reset_wait(self, timeout=None, seed=None, options=None):
        ...
        if not self.shared_memory:
            # gym >=0.26 takes (space, items, out); older releases took (items, out, space)
