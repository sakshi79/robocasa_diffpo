...
class RobomimicImageRunner(BaseImageRunner):
    ...
    def __init__(self, 
        ...
        assert n_train == 0, "Training envs not supported"
        ...
        env = AsyncVectorEnv(
            ...
            # robocasa envs expose an OrderedDict observation space, which gym treats as
            # a custom space and refuses to back with shared memory. Observations are
            # pickled over the worker pipes instead.
            shared_memory=False,
        )
    ...
    def close(self):
        if not isinstance(self.env, SyncVectorEnv):
            # AsyncVectorEnv owns worker processes. Close it here so they are reaped
            # deterministically; otherwise cleanup falls to VectorEnv.__del__, which may
            # not run until interpreter shutdown -- where the pipes are already torn
            # down and close_extras raises BrokenPipeError and leaks semaphores.
            self.env.close()
            return
